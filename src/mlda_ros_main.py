...
class MLDAMain():
    ...
    def judge_target_object_mlda(self, msg):
        if msg.status == "estimate":
            ...
            result = self.mlda_image.image_server(msg)
            if result == 0:
                return SendImageMLDAResponse(success = False, object_word = "apple", object_word_probability = 0.1)
            self.mlda_learn.mlda_server(msg)
            result = self.mlda_word.word_server(msg)
        ...
        for w in result.keys():
            key_start = w.find('_')
            #key_goal = len(w)
            target_object = w[key_start+1 :]
            object_word = target_object
            #print(object_word)
